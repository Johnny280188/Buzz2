...
CREATE OR REPLACE FUNCTION audit_changes() RETURNS TRIGGER AS $config_audit$
    BEGIN
        IF (TG_OP = 'DELETE') THEN
            INSERT INTO buzz99_demo.audit_config SELECT gen_random_uuid(), OLD.id, 'D', OLD.jsonb, current_timestamp;
            RETURN OLD;
        ELSIF (TG_OP = 'UPDATE') THEN
            INSERT INTO buzz99_demo.audit_config SELECT gen_random_uuid(), NEW.id, 'U', NEW.jsonb, current_timestamp;
            RETURN NEW;
        ELSIF (TG_OP = 'INSERT') THEN
            INSERT INTO buzz99_demo.audit_config SELECT gen_random_uuid(), NEW.id, 'I', NEW.jsonb, current_timestamp;
            RETURN NEW;
        END IF;
        RETURN NULL;
    END;
